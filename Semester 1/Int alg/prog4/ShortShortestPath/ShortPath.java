...
public class ShortPath  {
    ...
    public int shortestPath(Edge[] edges, int nodeCount,
                            int maxEdges, int sourceNode, int targetNode) {
        distance = new int[nodeCount];
        Arrays.fill(distance,Integer.MAX_VALUE);
        distance[sourceNode] = 0;
        for(int k = 0; k<maxEdges; k++){
            int[] dist = distance.clone();
            for(Edge e : edges){
                if(dist[e.from]<Integer.MAX_VALUE){
                    distance[e.to] = Math.min(distance[e.to],dist[e.from]+e.weight);
                }
            }
        }
