...
public class GUI {
    ...
    /**Shortcuts*/
    private int CTRL = Event.CTRL_MASK, CTRL_SHIFT = CTRL | Event.SHIFT_MASK;
    ...
    public GUI(){
        ...
        mainFrame.setJMenuBar(menuBar);                         //Sets the menubar.
    ...
    /**
     * Creates the menubar which contains two menus: Game menu and log menu.
     * Game menu has entires for: 
     *                  New game, shortcut: Ctrl+Shift+N,
     *                  Pause/resume game, shortcut: Ctrl+Shift+P,
     *                  Abort game, shortcut: Ctrl+Shift+A,
     *                  Options..., shortcut: Ctrl+Shift+O,
     *                  Set speed, a menu that contains buttons to change the speed of the game, a shortcut is Ctrl+[1;4] where 1 is slowest 
     *                  and 4 is fastest.
     *                  
     * Log Menu has entreis for:
     *                  Play log..., shortcut: Ctrl+Shift+L,
     *                  Save log..., shortcut: Ctrl+Shift+S
     * @return a JMenuBar containing two menus to control the game.
     */
    public JMenuBar createMenuBar(){
        //Initialize a menubar
        JMenuBar menuBarTemp = new JMenuBar();

        //Initialize menus
        JMenu gameMenu = new JMenu("Game");                                 //Gives a list of game options
        JMenu logMenu = new JMenu("Log");                                   //Gives a menu for playing and saving logs
        JMenu setSpeedMenu = new JMenu("Set speed");                        //Makes a menu for speeds.

        //Add the menus to the menubar
        menuBarTemp.add(gameMenu);
        menuBarTemp.add(logMenu);

        //Add four options to the gameMenu and add shortcuts. 
        JMenuItem newGameItem = new JMenuItem("New game");                  //Let's the player start at new game. 
        ...
        JMenuItem pauseResumeGameItem = new JMenuItem("Pause/Resume game"); //Makes it possible to Pause/resume the game
        gameMenu.add(pauseResumeGameItem);
        pauseResumeGameItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_P, CTRL_SHIFT));
        pauseResumeGameItem.addActionListener( e -> pauseResumeButton.doClick());

        JMenuItem abortGameItem = new JMenuItem("Abort game");              //?et's the player abort the game. 
        ...
        JMenuItem optionsGameItem = new JMenuItem("Options...");            //Show more options for the game
        gameMenu.add(optionsGameItem);
        optionsGameItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_O,CTRL_SHIFT));
        optionsGameItem.addActionListener( e -> optionsButton.doClick());

        //Add the speed menu to the gameMenu. 
        gameMenu.add(setSpeedMenu);

        //Add different speeds as an option to the speed menu and gives them shortcuts. 
        JMenuItem slowSpeedItem = new JMenuItem("slow");                    //Makes slow speed an option
        setSpeedMenu.add(slowSpeedItem);
        slowSpeedItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_1, CTRL));
        slowSpeedItem.addActionListener( e -> setSpeed(1));

        JMenuItem mediumSpeedItem = new JMenuItem("medium");                //Makes medium speed an option
        setSpeedMenu.add(mediumSpeedItem);
        mediumSpeedItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_2, CTRL));
        mediumSpeedItem.addActionListener( e -> setSpeed(2));

        JMenuItem fastSpeedItem = new JMenuItem("fast");                    //Makes fast speed an option
        setSpeedMenu.add(fastSpeedItem);
        fastSpeedItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_3, CTRL));
        fastSpeedItem.addActionListener( e -> setSpeed(3));

        JMenuItem sonicSpeedItem = new JMenuItem("sonic");                  //Makes sonic speed an option
        setSpeedMenu.add(sonicSpeedItem);
        sonicSpeedItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_4, CTRL));
        sonicSpeedItem.addActionListener( e -> setSpeed(4));

        //Add two options to the logMenu and gives them shortcuts.
        JMenuItem playLogItem = new JMenuItem("Play log...");               //Will let the player play a log eventually
        ...
        JMenuItem saveLogItem = new JMenuItem("Save log...");               //Will save a play log eventually
    ...
    private void applyOptions() {
        ...
        try{
            ...
            if(minLoss < 0 || maxLoss < 0 || minLoss > 100 || maxLoss > 100 ){
                JOptionPane.showMessageDialog(mainFrame, "'Min. loss' and 'Max. loss' must be between 0 and 100.", "Malformed input", JOptionPane.ERROR_MESSAGE);
                return;
            }
            if(minLoss > maxLoss){
                JOptionPane.showMessageDialog(mainFrame, "'Min. loss' must be smaller or equal to 'Max. loss'", "Malformed input", JOptionPane.ERROR_MESSAGE);
                return;
            }

        } catch (NumberFormatException e){
            JOptionPane.showMessageDialog(mainFrame, "'Min. loss' and 'Max. loss' must be integers.", "Malformed input", JOptionPane.ERROR_MESSAGE);
            return;
        }
    ...
    public JFrame createOptionsDialogBox(){
        ...
        JLabel minLossInterval = new JLabel("€ in [0,100]");
        tollAndRobberyPanel.add(minLossInterval);
        ...
        JLabel maxLossInterval = new JLabel("€ in [0,100]");
        tollAndRobberyPanel.add(maxLossInterval);
    ...
    /**
     * Plays the selected log.
     */
    private void playButton(){
        int chooser = fileChooser.showOpenDialog(mainFrame);
        if(chooser == JFileChooser.CANCEL_OPTION){}
        else if(chooser == JFileChooser.APPROVE_OPTION){
            try{ObjectInputStream os = new ObjectInputStream(
                                        new FileInputStream(
                                            fileChooser.getSelectedFile()));
                Log log = null;
                try{log = (Log) os.readObject();}
                catch(ClassNotFoundException e ){System.out.println("Shame!");}
                game.playLog(log);
                os.close();
            }
            catch(IOException e){
