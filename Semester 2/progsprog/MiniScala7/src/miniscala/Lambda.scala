...
object Lambda {
  ...
  def main(args: Array[String]): Unit = {
    println(encode(BoolLit(true)))
    println(encode(IntLit(2)))
    println(Unparser.unparse(encode(BinOpExp(encode(IntLit(0)),EqualBinOp(),IntLit(0)))))
    println(Unparser.unparse(encode(UnOpExp(NotUnOp(),BoolLit(true)))))
    println(Parser.parse("{val x = 2; x*2}"))
    println("6:" +Parser.parse("{val x = 6; x*12}"))
    println("This is x*2 & 2      "+ Unparser.unparse(encode(Parser.parse("x*2")))+" "+Unparser.unparse(encode(Parser.parse("2"))))
    println("This is val x= 2; x*2"+Unparser.unparse(encode(BlockExp(List(ValDecl("x",None,IntLit(2))),List(),BinOpExp(VarExp("x"),MultBinOp(),IntLit(2))))))
    println("This is val x= 6; x*12"+Unparser.unparse(encode(BlockExp(List(ValDecl("x",None,IntLit(6))),List(),BinOpExp(VarExp("x"),MultBinOp(),IntLit(12))))))
    println("This is x*12 & 6           "+ Unparser.unparse(encode(Parser.parse("x*12")))+" "+Unparser.unparse(encode(Parser.parse("6"))))
    println(encode(Parser.parse("true")))
    println("her:")
    val firsa = Parser.parse("0+1==0")
    println(firsa)
    val initialEnv = makeInitialEnv(firsa)
    println(Interpreter.eval(firsa,initialEnv))
    println("encoding")
    println(encode(firsa))
    println(Unparser.unparse(encode(firsa)))
    //println(Interpreter.eval(encode(firsa),makeInitialEnv(encode(firsa))))
    println("tester plus:")
    println(Unparser.unparse(encode(Parser.parse("0+1"))))
    println(Unparser.unparse(encode(Parser.parse("0"))))
    println(Unparser.unparse(encode(Parser.parse("1"))))
    println(Unparser.unparse(encode(Parser.parse("2"))))
    println(Unparser.unparse(encode(Parser.parse("0"))))
    println(encode(Parser.parse("if (!false) 1 else 2")))
  ...
  def encode(e: Exp): Exp =
    e match {
      ...
      case BinOpExp(leftexp, EqualBinOp(), IntLit(0)) =>   // e == 0, slide 18
        ...
        /**CallExp(leftexp,List(LambdaExp(List(FunParam("n",None)),
      ...
      case UnOpExp(op, subexp) =>
        op match {
          case NotUnOp() => // !e, slide 15
            encode(IfThenElseExp(subexp, BoolLit(false), BoolLit(true)))   //forsøg på encoding....
          case _ => // remaining cases are not (yet) implemented
            throw new EncoderError(e)
        }
  ...
  def decodeNumber(v: Val): Int = v match {
    case ClosureVal(params, _, exp, env,_) =>       //extra tomt stadie tilføjet
