...
object Test37 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // exercise 31
    var env2 = Map("x" -> IntVal(1), "y" -> TupleVal(List(IntVal(2), IntVal(3))))
    test("{ val z = 1; if (z == x) 42 else 17 }", IntVal(42), env2)
    test("y match { case (a, b) => x <= (a + b)}", BoolVal(true), env2)
    println("{ val x = (17, x == 2); y match { case (a, b) => { val z = (a + b, true); if (x == z) a - b else a + b } } }")
    println(Unparser.unparse(parse("{ val x = (17, x == 2); y match { case (a, b) => { val z = (a + b, true); if (x == z) a - b else a + b } } }"
    )))
    println(parse("{ val x = (17, x == 2); y match { case (a, b) => { val z = (a + b, true); if (x == z) a - b else a + b } } }"
    ))
    /* test("{ val x = (17, x == 2); y match { case (a, b) => { val z = (a + b, true); if (x == z) a - b else a + b } } }"
      , IntVal(5), env2) */
    /* println(eval(parse("if (x == z) a - b else a + b "),env2 + ("z" -> TupleVal(List(IntVal(5),BoolVal(true))))))*/
    println(eval(parse("y match { case (a, b) => { val z = (a + b, true) ; if (x == z) a - b else a + b }  }"),env2))
  }
