  ...
  def eval(e: Exp, env: Env, cenv: ClassEnv, sto: Sto): (Val, Sto) = e match {
    ...
    case BinOpExp(leftexp, op, rightexp) =>
      ...
      op match {
        ...
        case AndAndBinOp() =>
          (leftval,rightval) match {
            case (BoolVal(false),_)=> (BoolVal(false),sto2)
            case (BoolVal(true),BoolVal(true)) => (BoolVal(true),sto2)
            case(BoolVal(a),BoolVal(b))=> (BoolVal(false),sto2)
            case _ => throw new InterpreterError("Illegal and and operation, not boolean",op)
          }
        case OrBinOp() =>
          trace("Evaluating less than or equal")
          (leftval, rightval) match {
        ...
        case OrOrBinOp() =>
          (leftval,rightval) match{
            case (BoolVal(true),_) => (BoolVal(true),sto2)
            case (BoolVal(false),BoolVal(true)) => (BoolVal(true),sto2)
            case (BoolVal(false),BoolVal(false)) => (BoolVal(false),sto2)
            case _=> throw new InterpreterError("Illegal or or operation, not boolean",op)
          }
      }
    case UnOpExp(op, exp) =>
    ...
    case DoWhileExp(body,cond) =>
      ...
      eval(WhileExp(cond,body),env,cenv,sto1)
      ...
      eval(funexp, env,cenv, sto) match {
        case (ClosureVal(params, optrestype, body, kenv, cenv1,defs), sto1) =>
          ...
          if (args.length == params.length) {
            ...
            val ot = getType(optrestype,cenv) //dynamic typecheck?
            return res
          }
          else
  ...
  def checkValueType(v: Val, ot: Option[Type], n: AstNode): Unit = ot match {
    case Some(t) =>
      (v, t) match {
        ...
        case ( RefVal(loc,_),_) =>
          if(loc == -1) return
          throw new InterpreterError("Something wen horribly wrong",n)
        case _ =>
