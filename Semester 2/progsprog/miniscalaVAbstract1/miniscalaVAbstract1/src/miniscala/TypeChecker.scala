  ...
  def typeCheck(e: Exp, tenv: TypeEnv, ctenv: ClassTypeEnv): Type = e match {
    ...
    case BlockExp(vals, vars, defs, classes, exps) =>
      ...
      for (d <- vals) {
        ...
        checkSubtype(t,ot,d)
        tenv_updated += (d.x -> ot.getOrElse(t))
      }
      //VarsDecl old
      /**for(d <- vars){
        val dType = typeCheck(d.exp,tenv_updated,ctenv)     //theta e : tau
        checkTypesEqual(dType,d.opttype,BlockExp(vals, vars, defs, classes, exps))    //tau = type(tau)
        tenv_updated += (d.x->RefType(dType))         //theta' x -> ref(tau)
      } */
      //VarsDecl new
      for(d <- vars){
        val dType = typeCheck(d.exp,tenv_updated,ctenv)
        val ot = getType(d.opttype,ctenv,d)
        //checkTypesEqual(dType,ot,d)
        checkSubtype(dType,ot,d)
        tenv_updated += (d.x->RefType(dType))
      }
      ...
      for (d <- defs) {
        ...
        //checkTypesEqual(typeCheck(d.body, tenvy,ctenv), d.optrestype, BlockExp(vals,vars, defs,classes, exps))  //tau_2 = type(t_2) resttype
        checkSubtype(typeCheck(d.body, tenvy,ctenv), d.optrestype, BlockExp(vals,vars, defs,classes, exps))
      }
      //Class decl
      ...
      for(cd <- classes) {
        ...
        for (i <- cd.params.indices) {
          typeenvUpdated += (cd.params(i).x -> tau(i).getOrElse(throw new TypeError("",BlockExp(vals, vars, defs, classes, exps))))
        }
      ...
      var ctenvUpdated = ctenv
      for(cd <- classes){
        ctenvUpdated += (cd.klass -> getConstructorType(cd,ctenv,classes))
        var typeEnvUpdated = tenv
        for(x <- cd.params){
          typeEnvUpdated += (x.x -> getType(x.opttype,ctenvUpdated,BlockExp(vals, vars, defs, classes, exps)))
        }
    ...
    case AssignmentExp(x, exp) =>
      tenv(x) match{
        case RefType(a) => //checkTypesEqual(typeCheck(exp,tenv,ctenv),Some(a),e)
          checkSubtype(typeCheck(exp,tenv,ctenv),Some(a),e)
          return unitType
    ...
    case NewObjExp(klass, args) =>
      val constructor = ctenv.getOrElse(klass,throw new TypeError("No objexp",NewObjExp(klass, args)))
      if(args.length!=constructor.params.length){
          throw new TypeError("Wrong number of arguments",NewObjExp(klass, args))
      }
      for(i <-args.indices){
        val e = typeCheck(args(i),tenv,ctenv)
        //checkTypesEqual(e,constructor.params(i).opttype,NewObjExp(klass, args))
        checkSubtype(e,constructor.params(i).opttype,NewObjExp(klass, args))
      }
      constructor
    ...
    case LookupExp(objexp, member) =>
      typeCheck(objexp,tenv,ctenv) match {
        case ConstructorType(k,tau1,m) => getType( m.getOrElse(member,throw new TypeError("No type present",LookupExp(objexp, member))),ctenv)
        case _ => throw new TypeError("Not a constructorType",LookupExp(objexp, member))
      }
...
/**
  ...
  */
  def subtype(t1: Type, t2: Type): Boolean =
    ...
    case(ClassType(_),_) => false   //Jeppe "hack"
    case (_,ClassType(_)) => false
    ...
    case (FunType(left1,right1),FunType(left2,right2)) =>
      ...
      left1.zip(left2).forall(l => subtype(l._2,l._1)) && subtype(right1,right2) }
      else false
    case (TupleType(left1),TupleType(left2)) =>
      if (left1.length == left2.length) {
      left1.zip(left2).forall(l => subtype(l._1,l._2))}
      else false
    case _ => false
