...
import miniscala.TypeChecker.ConstructorType
...
object Unparser {
  ...
  def unparse(n: AstNode): String = n match {
    ...
    case ClassType(c) => c.toString
    case ConstructorType(_,_,_) => "Constructor type error"
    ...
    case UnOpExp(op, exp) =>
      ...
      op1+"("+exp1+")"
    case BlockExp(vals,vars,defs,classes,exp) => /** BlockExp(vals: List[ValDecl], vars: List[VarDecl], defs: List[DefDecl], classes: List[ClassDecl], exps: List[Exp])*/
      var valString = ""
      var endTuborg = ""
      for(d <- vals){
        valString += "{ " + unparse(d) + ";"
