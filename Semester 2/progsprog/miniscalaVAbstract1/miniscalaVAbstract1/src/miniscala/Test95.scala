...
object Test95 {
  ...
  def main(args: Array[String]): Unit = {
    testVal("{def x() = 7; def f(x) = x; f(2)}",IntVal(2))
    test("{def f(z: Int, t : Int, x: Int, y: Int): Int = if(y<=t) f(z+1,t-y,x,y) else z; f(0,17,17,5)}",IntVal(3),IntType())
    test("{ var x: Int = 0;def inc(): Int = { x = x + 1; x };inc();inc() }",IntVal(2),IntType())
    test("{ def make(a: Int): () => Int = {   var c: Int = a;   def inc(): Int = { c = c + 1; c };  inc };  { val c1 = make(100); val c2 = make(1000); c1() + c1() + c2() } }",IntVal(1204),IntType())
