...
object Week12 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(reverse(Cons(new Student(3), Cons(new Student(2),Nil))))
    println(reverse(Cons(new Student(3), Cons(new Student(5),Nil))))
  ...
  def boringMerge[T<:Comparable[T]](xs: List[T],ys: List[T]): List[T] = xs match{
  ...
  def randomIntList(): List[Student] = randomStudentListt(Nil,new Random().nextInt(101))
  def randomStudentListt(ass : List[Student],n:Int): List[Student] = {
    if(n>0) {
      val p: Int = new Random().nextInt()
      val s: Student = new Student(p)
      randomStudentListt(Cons(s, ass), n - 1)
    }
    else
      ass
  }
